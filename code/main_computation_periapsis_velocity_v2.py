...
@file: main_computation_periapsis_velocity_v2.py
@date: 9/17/23 16:55
@desc: 
"""
...
def gen_sample(Objfile, friction):
    # Read OBJ data
    Objpath = "./data/"
...
if __name__ == '__main__':
    ...
    friction = 0.5
    max_radius = gen_sample(Objfile, friction)
    ...
    Rec_info = np.zeros([RowNum*ColNum,7], dtype=np.float32)
    ...
    for i in range(RowNum):
        for j in range(ColNum):
            ...
            Rec_info[j+i*ColNum,0] = (periapsis_min + periapsis_step*i)
            Rec_info[j+i*ColNum,1] =  vel_inf/1E3 # km/s
            Rec_info[j + i * ColNum, 2] = 1/wbeg
            Rec_info[j + i * ColNum, 3] = info[-1, 1] # loss number
            Rec_info[j + i * ColNum, 4] = info[-1, 2]  # nomove number
            Rec_info[j + i * ColNum, 5] = info[-1, 3]  # shifting number
            Rec_info[j + i * ColNum, 6] = info[-1, 4]  # orbit number
    ...
    # Row index: 1. periapsis, 2. vel_inf, 3. period ratio, 4. loss number, 5. nomove number, 6. shifting number, 7. orbit number
    out_info.to_csv('./data/pervel3b_info.txt', sep=',', index=False, header=False)
