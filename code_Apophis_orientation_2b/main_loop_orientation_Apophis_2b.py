...
def computation(max_radius):
    ...
    # max_radius = np.array([0.7355706, 0.5282567, 0.5068329, 0.49067935])
...
if __name__ == '__main__':
    ...
    alpha_max = np.pi
    alpha_min = 0
    alpha_step = (alpha_max - alpha_min) / (RowNum - 1)
    beta_max = 2*np.pi
    beta_min = 0
    beta_step = (beta_max - beta_min) / (ColNum - 1)
    ...
    for i in range(RowNum):
        for j in range(ColNum):
            ...
            bodies, max_radius = gen_sample(Objpath, Objfile, alpha, beta, fric=0.5, rstn=0.55, rstt=0.55)
            info = computation(max_radius)

            Rec_info[j + i * ColNum, 0] = np.rad2deg(alpha) # deg
            Rec_info[j + i * ColNum, 1] = np.rad2deg(beta)  # deg
    ...
    # Row index: 1. alpha, 2. beta, 3. Id/Id0, 4. we0/we, 5. H/H0, 6. info
    out_info.to_csv('./data/ds_orientation_Apophis_2b_v1.txt', sep=',', index=False, header=False)

    # 获取文件名（含后缀）
    name = os.path.basename(__file__)
    path = os.getcwd()  # 获取当前工作目录路径
    mesg = 'Finish: ds_orientation_Apophis_2b_v1'
